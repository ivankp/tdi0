...
games = {
    'columns': [
        ...
        'poll_age','poll_numpl',
        'num_rated', 'rating', 'bayes_rating', 'owned'
    ],
    'index': index,
    'data': data
}
...
for fname in sorted(glob('../project/bgg_*.tar.gz'),key=split_ints):
    print(fname)
    with tarfile.open(fname) as tar:
        for f in tar:
            print(f.name)
            for g in etree.fromstring(tar.extractfile(f).read()).xpath('/boardgames/boardgame'):
                x = g.get('objectid')
